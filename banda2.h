...
void cruzar(int * data)
{
	...
	if(sentido==IZQUIERDA && capacidad>onBoard && izquierda >0)
	{
		...
		if(decide())
		{
			...
			if(actual <(bandas-1))
			{
			    printf("thread %d: voy a mandar persona a la izquierda de %d\n", actual, actual+1);
			    msync (FILEPATH, bandas*2*sizeof(int), MS_ASYNC);
				data[((actual+1)*2)]+=1;
				write(FILEPATH,&data,getpagesize());
				msync (FILEPATH, bandas*2*sizeof(int), MS_ASYNC);
		...
		if(decide())
		{
			...
			if(actual >(0))
			{
			    printf("thread %d: voy a mandar una persona a la derecha de %d\n", actual, actual-1);
    			//msync (FILEPATH, bandas*2*sizeof(int), MS_ASYNC);
...
//		derecha = data[actual*2+1];
		...
		printf("hostname= %s, thread =%d, i=%d , d=%d\n",hostname, actual,data[actual*2],data[actual*2+1]);
		//printf("hostname= %s, thread =%d, i=%d , d=%d\n",hostname, actual, izquierda, derecha);
		sleep(4);
		generamela();
